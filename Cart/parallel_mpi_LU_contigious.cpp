...
void serial_LU(double **matrix, int dim)
{
	...
	printf("L from serial\n \n ");
	mat_print(Lower,dim,dim);
	printf("U from serial\n \n ");
	mat_print(matrix,dim,dim);
...
//
//
//NEED TO CREATE A SQUARE MESH OF PROCS
...
int main(int argc, char **argv)
{
	...
	if (argc<2){
		if (rank ==0){
			printf("Number of arguments must be 3 -- No. of procs, dimension and number of threads");
		}
	}
	...
	int row=dim;
	int col=dim;
	//arma::mat A=arma::zeros(row,col);
	//A=arma::randi<arma::mat>(row,col,arma::distr_param(1,5));
	double **mat=NULL;
	double **A=NULL;
...
//	printf("mat \n\n");
//	mat_print(mat,dim,dim);
//	printf("\n\n");
	cart_LU(argc,argv,mat,dim,nblocks,num_threads);
	if(rank ==0){
//		printf("This is the Cartesian verison from main \n");
		printf("A \n\n");
		mat_print(A,dim,dim);
		printf("\n\n");
		serial_LU(A,dim);
