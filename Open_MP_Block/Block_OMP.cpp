...
void block_LU(double *mat, int  M, int  N, int  NBlocs, int  *permut){
	...
	double *store_kk=store+k*NBlocs*M+k*NBlocs*NBlocs;
        ...
        #pragma omp task depend(inout: store_kk[0:row_chunk*NBlocs]) \
       		         depend(out: permut[k*NBlocs:NBlocs]) \
			 firstprivate(store_kk,row_chunk)
	{
	...
	for (int j=k+1; j<col_tiles;j++){
        ...
        #pragma omp task depend(inout: store_kj[0:NBlocs*NBlocs]) depend(in: store_kk[0:row_chunk*NBlocs]) depend(in: permut[k*NBlocs:NBlocs]) firstprivate(store_kk,store_kj,rem_row_chunk, row_chunk)
        {
     ...
     for(int ii=1; ii<col_tiles;ii++){
	    int start=ii*NBlocs;
	    int end=N;
       #pragma omp task depend(in:permut[(col_tiles-1)*NBlocs:NBlocs]) \
		    	    depend(inout:store[ii*NBlocs*M:M*NBlocs])
	   {
...
//********************
	...
	printf("Time in Microsecond using PAPI%lld\n",(end_usec-start_usec));
